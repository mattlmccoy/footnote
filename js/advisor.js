...
import { wordDiff } from './textdiff.js?v=e7ceb8b';
import { loadConfig, dataRepoParts, loadChapters, setConfig, dataRepoFromParams, workspaceInviteBroken } from './config.js?v=e7ceb8b';   // instance config + chapter manifest; assistant-free by construction
import { keyFromSearch, searchWithoutKey } from './invite.js?v=e7ceb8b';   // magic-link: key in the invite URL
import { makeSafeStore } from './safestore.js?v=e7ceb8b';   // never-throw storage so a blocked browser can't kill boot (F4)
import { parseVersion, latestFromHtml, isStale } from './version.js?v=e7ceb8b';   // stale-bundle refresh nudge
import { reviewingHeader, releaseView, validateKey, FIRST_RUN_TOUR, commentDraftKey } from './onboarding.js?v=e7ceb8b';   // pure onboarding logic (header/state routing/key validation/first-run guide/draft key)
import { orderedUnits, mergeReviews as flattenReviews, routeWrite, wrapUnit, stripSegmentId } from './wholedoc.js?v=e7ceb8b';   // whole-document reader mirror (used on render + comment paths) — DO NOT drop; a bad merge once did and broke the reviewer
import { startWatch as startNetWatch } from './netstatus.js?v=e7ceb8b';
import { showBuildTag } from './buildinfo.js?v=e7ceb8b';
import { readProgress } from './cardstats.js?v=e7ceb8b';   // shared read-progress derivation (parity with author cards)
import { fetchWithTimeout, classifyGitHubError, retryAfterMs, TTLCache, orphanComments } from './nethelpers.js?v=e7ceb8b';   // bounded fetch + rate-limit backoff + read cache + orphan fallback
startNetWatch();
