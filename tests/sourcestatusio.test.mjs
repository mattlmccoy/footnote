...
test('fetchSourceStatus: no built.json yet -> notbuilt (needs a rebuild), not false-healthy', async () => {
  const routes = [['commits/main', { sha: 'HEAD', commit: { committer: { date: DATE } } }]];
  const s = await fetchSourceStatus({ token: 't', sourceRepo: 'me/src', dataRepo: 'me/data', fetchImpl: fakeFetch(routes) });
